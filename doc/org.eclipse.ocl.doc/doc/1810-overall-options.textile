h3(#OverallOptions). OverallOptions
...
This URI was introduced in the Juno release.
...
EMF Delegate annotations referencing the @http://www.eclipse.org/emf/2002/Ecore/OCL@ URI are serviced by the evaluator selected on the preference page.
...
Selecting the default delegation mode retains the Helios and Indigo functionality whereby *genmodel* encodes the OCL expressions as text strings in a generated model. Each expression is lazily compiled at run-time with the result being cached to reduce overheads for repeated execution.
