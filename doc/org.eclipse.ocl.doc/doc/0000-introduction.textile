...
Eclipse OCL is an implementation of the OMG OCL 2.3.1 specification for use with Ecore and UML meta-models.
...
Invoke *File->New->Project...* then select *Examples* then *OCL (Object Constraint Language) Plugins* then *OCLinEcore Tutorial* and *Finish* to create a small example project called *OCLinEcoreTutorial*. It contains
...
The preliminary direct Java code generator is described in the "Code Generator Tutorial":#CodeGenerationTutorial.
...
Eclipse OCL is used as a component in a variety of other Eclipse projects such as Acceleo, BIRT, GMF, Modisco, QVTo. Ecore models are used directly, which leads to some internal difficukties.

A new Eclipse OCL is evolving that exploits Xtext and uses Ecore models indirectly via a UML-aligned Pivot models. This provides a choice between the mature APIs that offer limited forward functionality, and the experimental new APIs that will be promoted to non-experimental form in the Kepler release.
...
Full model-driven extensibility is planned for the Kepler release.

h2. Who Uses OCL and Eclipse OCL?

The OCL specification is defined by the OMG(Object Management Group). It originally evolved as part of UML where it is used for the detailed mathematical aspects that are not appropriate for graphical exposition. OCL 2.0 was split off from UML 2.0 in recognition of its greater utility.
...
The use of OCL for execution will be enhanced by providing direct Java code generation for OCL embedded in models. This should alleviate many of the performance concerns for interpreted execution of embedded OCL.
