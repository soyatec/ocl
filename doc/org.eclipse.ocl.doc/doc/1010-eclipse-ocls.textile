...
In the Kepler (4.1) release, the transition should be complete and many org.eclipse.ocl.examples.* plugins will be renamed to org.eclipse.ocl.*. All functionality should use the new Pivot metamodel by default. However the mature Ecore and UML support will remain for compatibility.
